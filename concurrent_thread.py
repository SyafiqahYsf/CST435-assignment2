...
OUTPUT_DIR = "output_concurrent_thread"          # Folder where processed images will be saved
WORKER_COUNTS = [1, 2, 4, 8]         # Number of parallel threads to test
...
def apply_image_filters(data):
    ...
    # --- Sequential Pipeline ---
    # Each task is applied one after another.
    ...
    output_path = os.path.join(output_folder, f"thread_{file_name}")
    cv2.imwrite(output_path, final_image)
    
    return file_name

# ---------------- Run Processing using ThreadPoolExecutor ----------------
def run_thread_test(image_paths, workers):
    """
    Executes image processing using concurrent.futures with threads.
...
if __name__ == "__main__":
    ...
    if os.path.exists(INPUT_DIR):
        ...
        if not all_images:
            ...
            print(f"🚀 Starting Thread-Based Concurrent Processing on {len(all_images)} images...")
            results = {}

            # Loop through different numbers of threads for performance testing
            for count in WORKER_COUNTS:
                print(f"🔄 Testing with {count} thread(s)...")
                exec_time = run_thread_test(all_images, count)
            ...
            # Serial execution time (1 thread) is used as baseline for speedup calculation
            t_serial = results[1]
            for p in WORKER_COUNTS:
                t_p = results[p]
                speedup = t_serial / t_p                 # Speedup = serial / parallel
                efficiency = (speedup / p) * 100        # Efficiency = speedup / threads * 100%
                print(f"{p:<10} | {t_p:<12.4f} | {speedup:<10.2f} | {efficiency:<15.2f}%")
            print("="*65)
            print("✅ Performance data collected using ThreadPoolExecutor.")
    else:
